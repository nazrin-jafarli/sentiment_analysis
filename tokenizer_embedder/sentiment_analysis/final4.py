...
with torch.no_grad(): # bu hisse deqiq dogrudur
...
num_classes = 2
one_hot_train_labels = F.one_hot(train_labels, num_classes=num_classes)
one_hot_val_labels = F.one_hot(val_labels, num_classes=num_classes)
...
for epoch in range(num_epochs):
    ...
    train_loss = criterion(outputs, torch.argmax(one_hot_train_labels, dim=1))  # Use one-hot encoded labels
    train_loss.backward()
    optimizer.step()
    ...
    with torch.no_grad():
        ...
        val_loss = criterion(val_outputs, torch.argmax(one_hot_val_labels, dim=1))
