...
def main():
    ...
    # "last_hidden" or "average_all_hidden" chosen method for extracting features
    extract_method = "last_hidden" 
    ...
    # Generate embeddings for positive and negative class data
    def generate_embeddings(padded_data, attention_mask,extract_method):
        embeddings = []
        with torch.no_grad():
            for input_ids, mask in zip(padded_data, attention_mask):
                ...
                if extract_method == 'last_hidden':
                    last_hidden_state = outputs.hidden_states[-1]
                    embeddings.append(last_hidden_state.squeeze(0))

                elif extract_method == 'average_all_hidden':
                    hidden_states = outputs.hidden_states
                    num_layers = len(hidden_states)
                    layerwise_hidden_states = torch.stack(hidden_states)
                    avg_hidden_states = torch.mean(layerwise_hidden_states, dim=0)
                    embeddings.append(avg_hidden_states.squeeze(0))

        return torch.stack(embeddings)


    positive_embeddings = generate_embeddings(positive_padded, positive_attention_mask,extract_method)
    # print('positive_embeddings',positive_embeddings)
    negative_embeddings = generate_embeddings(negative_padded, negative_attention_mask,extract_method)
    neutral_embeddings = generate_embeddings(neutral_padded, neutral_attention_mask,extract_method)
    unlabeled_embeddings = generate_embeddings(unlabeled_padded, unlabeled_attention_mask,extract_method)

    # Combine positive,neutral and negative embeddings and also labels
    labeled_embeddings = torch.cat([negative_embeddings,neutral_embeddings,positive_embeddings])
    # Assign labels: 0 for negative, 1 for neutral, and 2 for positive
    ...
    kmeans_semi_supervised = KMeansSemiSupervised(n_clusters=3, max_iter=100,distance_measure=dist_measure) #distance_measure 'cosine' or 'euclidean' 
    ...
    def train_classifier(model, criterion, optimizer, train_loader, val_loader, num_epochs):
        train_losses = []
        val_losses = []
        with open("epoch_results.txt", "w") as epoch_file:
            epoch_file.write("Epoch, Train Loss, Val Loss, Val Accuracy\n")
            for epoch in range(num_epochs):
                ...
                predicted_list = []   # List to store predicted labels
                with torch.no_grad():
                ...
                epoch_file.write(f'{epoch+1}, {train_loss}, {val_loss}, {val_accuracy}\n')
    ...
    def evaluate_model(model, criterion, test_loader):
        ...
        test_accuracy = accuracy_score(true_labels, predicted_labels)  # Calculate accuracy using accuracy_score()
